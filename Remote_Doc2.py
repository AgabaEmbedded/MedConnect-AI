...
def main():
    """
    Main function with interactive conversation demo.
    """
    
    # ==================== CONFIGURATION ====================
    # Replace with your actual Gemini API key
    api_key = "AIzaSyCQ_L0CYz3t12a5J3uGobgcLu6DaVxJ6_0"
    
    if api_key == "YOUR_GEMINI_API_KEY_HERE":
        print("⚠️  Please set your Gemini API key in the code!")
    ...
    while True:
        ...
        if user_input.lower() in ['quit', 'exit', 'bye']:
            print("\n👋 Thank you for using our medical assistant. Take care!")
            break
        ...
        try:
            ...
            if state.get("active_node"):
                print(f"\n📊 [Active Node: {state['active_node']}]", end="")
            ...
            if state.get("conversation_ended"):
                ...
                if state.get("soap_summary"):
                    ...
                    print("="*70)
                    print(state["soap_summary"])
                
                if state.get("matched_doctor"):
                    doc = state["matched_doctor"]
                    print("\n" + "="*70)
                    print("👨‍⚕️ MATCHED DOCTOR:")
                    print("="*70)
                    print(f"Name: Dr. {doc['name']}")
                    ...
                    print(f"Available: {doc['available_slots'][0]}")
                    print("="*70)
                
                # Ask if they want to start new conversation
                restart = input("\n🔄 Start a new conversation? (yes/no): ").strip().lower()
                if restart in ['yes', 'y']:
                    state = None
                    conversation_count = 0
    ...
    print(f"\n📊 Total interactions: {conversation_count}")
    print("="*70 + "\n")
